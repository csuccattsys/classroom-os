...
export default function StudentPortal() {
  ...
  // ================= MOCK DATA / API FETCH REPLACEMENT =================
  useEffect(() => {
    // Simulating API payload fetch for Caraga State University Cabadbaran Campus Portal
    const fetchPortalData = async () => {
      try {
        setLoading(true);
        // Replace this mock data array with your actual Axios/Fetch response payload
        const mockData = [
          {
            id: 1,
  ...
  const categories = ['All', 'Academic Advisory', 'USG Announcement', 'General Advisory', 'Campus Update'];

  return (
    <div className="min-h-screen bg-slate-50/50 text-slate-800 antialiased selection:bg-emerald-500/20 selection:text-emerald-900">
      
      {/* GLOBAL PORTAL TOP HEADER BRANDING */}
      <header className="sticky top-0 z-40 w-full border-b border-slate-200 bg-white/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-xl bg-emerald-600 flex items-center justify-center text-white font-black text-sm shadow-md shadow-emerald-600/20">
              C
            </div>
            <div>
              <h1 className="text-xs font-black uppercase tracking-wider text-slate-900">CSUCC Student Hub</h1>
              <p className="text-[10px] text-slate-400 font-medium">Caraga State University Cabadbaran Campus</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 pl-1">
              <div className="h-7 w-7 rounded-full bg-slate-200 flex items-center justify-center font-bold text-xs text-slate-600 border border-slate-300">
                S
              </div>
            </div>
          </div>
      ...
      {/* PORTAL NAVIGATION AND CONTROLS SUB-HERO */}
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-2 md:pb-0 scrollbar-none">
            <button 
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 shrink-0 ${activeTab === 'announcements' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}
            >
              <LayoutDashboard className="h-3.5 w-3.5" /> Bulletin Boards
            </button>
            <button 
              onClick={() => setActiveTab('resources')}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 shrink-0 ${activeTab === 'resources' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}
            >
              <BookOpen className="h-3.5 w-3.5" /> Handbooks & Guides
            </button>
          </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              <div className="relative flex-1 sm:w-64">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-4 py-1.5 text-xs focus:outline-none focus:border-emerald-500 focus:bg-white transition-all font-medium text-slate-700"
                />
              </div>
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="appearance-none bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-8 py-1.5 text-xs font-bold text-slate-600 focus:outline-none focus:border-emerald-500 focus:bg-white transition-all cursor-pointer"
                >
                  {categories.map(cat => (
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {activeTab === 'resources' ? (
          <div className="bg-white border border-slate-200 p-8 rounded-2xl text-center text-slate-400 text-xs">
            Institutional Student Handbook PDFs and Regulatory Timelines will be visible here.
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="space-y-4">
                {loading ? (
                  <div className="text-center p-12 bg-white rounded-2xl border border-slate-200 text-slate-400 text-xs font-medium animate-pulse">
                    Synchronizing live campus data registry...
                  </div>
                ) : filteredItems.length === 0 ? (
                  <div className="text-center p-12 bg-white rounded-2xl border border-slate-200 text-slate-400 text-xs font-medium">
                    No active announcements or advisories matched your selection.
                  </div>
                ) : (
                  <div className="space-y-4">
                    {filteredItems.map((item, index) => (
                      <div key={item.id || index} className="bg-white border border-slate-200 p-5 rounded-2xl space-y-3.5 shadow-sm transition-all duration-200 hover:border-slate-300">
